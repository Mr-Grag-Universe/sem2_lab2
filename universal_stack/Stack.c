    ...
    //stack->head->item = x;
        ...
        switch (stack->type) {
            case 0:
                ...
                break;
            case 1:
                stack->head->item = malloc(sizeof(short int));
                ((short int*) stack->head->item)[0] = *((short int*) x);
                break;
            case 2:
                stack->head->item = malloc(sizeof(int));
                ((int*) stack->head->item)[0] = *((int*) x);
                break;
            case 3:
                stack->head->item = malloc(sizeof(long int));
                ((long int*) stack->head->item)[0] = *((long int*) x);
                break;
            case 4:
                stack->head->item = malloc(sizeof(long long int));
                ((long long int*) stack->head->item)[0] = *((long long int*) x);
                break;
            case 5: {
                char * str = (char *) x;
                size_t len = strlen(str);
                stack->head->item = malloc(sizeof(char) * (len+1));
                memmove(stack->head->item, str, sizeof(char) * (len+1));
                break;
            }
            default:
                break;
...
void stack_delete_el(Stack * stack) {
...
TYPE1 stack_get(Stack * stack) {
    if (stack->number_of_elements == 0) {
        fprintf(stderr, "\nThis stack is empty!\n");
        exit(STACK_IS_EMPTY);
    }

    // size_t res = (size_t) stack->head->item;
